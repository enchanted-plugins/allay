...
rm -f "/tmp/emu-drift-${SESSION_HASH}.jsonl" "/tmp/emu-drift-cooldown-${SESSION_HASH}"
...
if [[ ! -f "/tmp/emu-drift-${SESSION_HASH}.jsonl" ]]; then
...
LINE_COUNT=$(wc -l < "/tmp/emu-drift-${SESSION_HASH}.jsonl" | tr -d ' ')
