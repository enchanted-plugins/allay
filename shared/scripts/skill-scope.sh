...
#     started_at + EMU_SKILL_TTL is past. Both guards — `kill -0` fails
...
LOCK_DIR="${STATE_FILE}${EMU_LOCK_SUFFIX}"
...
cmd_register() {
  ...
  worktree="${EMU_WORKTREE_REL:-}"
  session_id="${EMU_SESSION_ID:-}"
  ...
  # is required.
  printf "export EMU_SCOPE_ID=%s\nexport EMU_SCOPE_PARENT=%s\nexport EMU_SCOPE_DEPTH=%s\nexport EMU_SCOPE_SKILL=%s\n" \
...
cmd_unregister() {
  ...
  # EMU_SCOPE_ID because in nested flows it may refer to a child scope).
...
# tr -d '\r' defends against jq's CRLF output on Windows stdout.
cmd_current_env() {
  acquire_lock "$LOCK_DIR" || { printf "EMU_SCOPE_SKILL=manual\nEMU_SCOPE_PLUGIN=\nEMU_SCOPE_ID=\nEMU_SCOPE_PARENT=\nEMU_SCOPE_DEPTH=0\n"; return 0; }
  local cleaned
  cleaned=$(_sc_read_clean)
  _sc_write_atomic "$cleaned" || true
  release_lock "$LOCK_DIR"
  ...
  printf "%s" "$cleaned" | jq -r '
    ...
    "EMU_SCOPE_SKILL=\($t.skill_id // "manual")",
    "EMU_SCOPE_PLUGIN=\($t.plugin // "")",
    "EMU_SCOPE_ID=\($t.invocation_id // "")",
    "EMU_SCOPE_PARENT=\($t.parent_invocation_id // "")",
    "EMU_SCOPE_DEPTH=\($t.depth // 0)"
  ' 2>/dev/null | tr -d '\r'
}
