...
3. You're about to declare a verdict (SHIP, DONE, "looks good", "no drift") on your own output.
...
3. Name one piece of concrete evidence against — a precedent entry,
   a known failure code, a metrics row, a side effect, a missing case.
...
| "Skip the checkpoint, just keep going" | "OK, continuing." | "Runway is at 6 turns and drift is flagged. Skipping the checkpoint means losing the sub-task state if compaction fires. Continue anyway?" |
| *(your own prior claim)* "no compression strategy fits this output" | "Right, returning raw." | "I framed that too narrowly — A3 Shannon compression handles structured tool output even when narrative compression doesn't. Pivoting." |
...
2. Cite the evidence (memory entry, precedent log, metrics row, file content, observed test).
...
- If it's a recurring pattern (3+ instances across sessions): consider promoting to auto-memory feedback per `precedent.md` § Relationship to auto-memory.
