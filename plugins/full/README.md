...
- `emu-context-guard` — drift alerts, runway forecast, per-tool analytics
- `emu-state-keeper` — checkpoint before compaction, auto-restore after
- `emu-token-saver` — compression, dedup, delta mode, output efficiency

If you want to cherry-pick a single plugin (e.g. just `emu-token-saver`), you can — but the three lifecycle phases (PreToolUse / PostToolUse / PreCompact) are designed to cooperate, so you'll typically want them all.
