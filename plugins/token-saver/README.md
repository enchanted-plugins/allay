...
2. **Re-read, same hash**: Advisory only — emit a stderr note ("would have blocked, here's the cached preview, prefix `FULL:` to suppress"). The Read still proceeds per [`../foundations/packages/core/conduct/hooks.md`](../../../foundations/packages/core/conduct/hooks.md) advisory contract.
