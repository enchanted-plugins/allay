...
Every tool call is attributed to the currently-active skill via a LIFO stack. Skills register scope at entry with a 16-hex-char invocation ID (surviving PID reuse per systemd convention). Attributes persist to `skill-metrics.jsonl` alongside `metrics.jsonl`. Emitted independently of standard metrics for fine-grained per-skill breakdown via `/emu:analytics`.
