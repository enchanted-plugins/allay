...
- The hook timed out. Hooks have per-event budgets; see [shared/foundations/conduct/hooks.md](../shared/foundations/conduct/hooks.md) § Performance budget.
...
**Symptom:** `/plugin install full@emu` succeeds but `/plugin list` doesn't show the sub-plugins.
...
```
/plugin marketplace remove enchanter-ai/emu
...
**Cause:** Someone's error payload is unhelpful — see [shared/foundations/conduct/tool-use.md](../shared/foundations/conduct/tool-use.md) § Error payloads are contracts.
...
- **Bug:** reproducible, unexpected behavior. File in [Issues](https://github.com/enchanter-ai/emu/issues) with the bug template.
- **Question or "is this expected?":** open a thread in [Discussions](https://github.com/enchanter-ai/emu/discussions).
...
Then: [Discussions Q&A](https://github.com/enchanter-ai/emu/discussions/categories/q-a).
